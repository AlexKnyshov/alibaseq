...
optional.add_argument('-d', metavar='namedelim', help='sample/contigID delimiter',dest="namedelim", default="|")
...
def actual_reciprocator(inpdict, recip_overlap, bstrands2, filtration_table2, metric, metricR):    
    ...
    for bad_element in blacklisted:
        print >> debugfile, bad_element[0],returndict[bad_element[0]][bad_element[1]]
            ...
            if targetkey1 in rec_dict: #checking for best contig for Q in current sample
                ...
                for rec_target, rec_hits in rec_dict[targetkey1].items():
                    if ref_hs1:
                        ...
                        if getOverlap(sample_t_region, reciprocal_q_region, recip_overlap) > recip_overlap:
                            # the first overlapping region to consider
...
def bltableout(output, bltableout_file, table_type):
    if table_type == "target":
        ...
        print >> bltableout_file, "query,supercontig,hit,PCindex,direction,PCstart,PCend,Qstart,Qend,Eval,Bit,Ident"
        for key, value in sorted(output.items()):
            for superCTG in value:
                for pseudoCTG in superCTG[1][3]:
                    if type(pseudoCTG) is not int:
                        ...
                        print >> bltableout_file, key+","+str(superCTG[0])+","+CTGname+","+pseudoCTGindex+","+pseudoCTGdir+","+pseudoCTGranges+","+pseudoCTGscores
...
messagefunc("contignum parameter: "+str(contignum), cols, debugfile_generic, False)
...
for b in blastlist:
    ...
    if loghead:
        print >> tout, "hit,num_queries,names_of_queries"
